...
const getEntry = (disruption, currentTimeUnix) =>
  disruption.filter((entry) => {
    ...
    if (window.location.pathname === "/currentDisruptions.html") {
      ...
      if (
        currentTimeUnix >= disruptionStart &&
        currentTimeUnix <= disruptionEnd &&
        route !== undefined
      ) {
        return entry;
      }
...
const renderDisruptions = async () => {
  ...
  if (disruptions !== undefined) {
    ...
    const filteredArray = filteredArrayRaw
      .sort((a, b) => {
        if (
          (a.start_dttm ?? Number.MAX_VALUE) >
          (b.start_dttm ?? Number.MAX_VALUE)
        )
          return -1;
        if (
          (a.start_dttm ?? Number.MAX_VALUE) <
          ...
          !accumulator.find(
            (item) =>
              item.description_text.replace(/ \nUse$/, "") ===
                current.description_text.replace(/ \nUse$/, "") &&
      ...
      .sort((a, b) => {
        if ((a.route_id ?? Number.MAX_VALUE) < (b.route_id ?? Number.MAX_VALUE))
          return -1;
        if ((a.route_id ?? Number.MAX_VALUE) > (b.route_id ?? Number.MAX_VALUE))
          return 1;
        return 0;
    ...
    const newLi = document.createElement("li");
    newLi.classList.add("apiError");
    newLi.innerHTML = `This City of Edmonton API is undergoing temporary maintenance. This issue should be resolved soon. Please check back later`;
